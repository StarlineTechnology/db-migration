...
while ($customer = $customers->fetch_assoc()) {
    ...
    if (!$prestashop_db->query($query)) {
        echo "<span style='color:red'>Error inserting customer ID $id_customer: " . $prestashop_db->error . "</span><br>";
    } else {
        echo "<span style='color:green'>Customer ID $id_customer migrated successfully.</span><br>";
    }
    // Make backup for oscommerce password.

    //echo "SELECT count(*) FROM  `zc_legacy_passwords` WHERE `email` = '$email'"; die;
    $customer = $prestashop_db->query("SELECT count(*) FROM `zc_legacy_passwords` WHERE `email` = '$email'");
    if ($customer){
        echo "<span style='color:blue'>Customer already exists </span><br>";
    }else{
